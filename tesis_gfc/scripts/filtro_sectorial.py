...
Clasificacion sectorial de los proyectos de carbono y filtro AFOLU.

Este modulo NO se ejecuta solo: lo importan 28_clasificar_sector.py (que
construye la tabla de decision) y 26_consolidar_fuentes_carbono.py (que la
aplica).

CRITERIO DE INCLUSION
---------------------
Se conserva un proyecto si su mecanismo de mitigacion declarado opera sobre
cobertura boscosa o uso del suelo (AFOLU). Los demas se excluyen porque la
...
clasificar nunca entra al tratamiento por omision.

RUTAS
-----
Todo se ancla a la carpeta del proyecto, deducida de la ubicacion de este
archivo (se asume que vive en tesis_gfc/scripts/). NO se usa el directorio
de trabajo: existe una carpeta data/interim duplicada bajo scripts/ que ha
causado que Python y R lean archivos distintos.
"""
...
# scripts/filtro_sectorial.py -> scripts/ -> tesis_gfc/
RAIZ_PROYECTO = Path(__file__).resolve().parent.parent
DIR_INTERIM = RAIZ_PROYECTO / "data" / "interim"

CLASIFICACION_FILE = DIR_INTERIM / "clasificacion_sectorial.csv"
...
def avisar_carpeta_sombra() -> None:
    """La existencia de <cwd>/data/interim cuando el cwd no es la raiz del
    proyecto es la causa de que 26 y 01 vean tratamientos distintos."""
    sombra = Path.cwd() / "data" / "interim"
    if sombra.exists() and sombra.resolve() != DIR_INTERIM.resolve():
        print("\n" + "!" * 70)
        print("AVISO: existe una carpeta de datos duplicada en")
        print(f"  {sombra}")
        print("distinta de la que usa este script:")
        print(f"  {DIR_INTERIM}")
        print("Esa duplicacion es la causa de que el pipeline de Python y la")
        print("cadena de R vean variables de tratamiento diferentes.")
        print("Borrala o renombrala cuando termines de correr esto.")
        print("!" * 70 + "\n")
...
# RENARE: el campo 'tipo' clasifica los registros sin ambiguedad.
#   'Proyecto REDD+' y 'Programa REDD+' son forestales por definicion.
#   PDBC, MDL y NAMA agrupan todo lo demas -- pero PDBC puede contener
#   foresteria legitima, asi que va a REVISAR mirando 'actividades'.
RENARE_TIPO_AFOLU = [r"REDD\+"]

# Verra: el alcance sectorial 14 es AFOLU. Las metodologias VM00xx y
# AR-ACM/AR-AM cubren forestal.
VERRA_AFOLU = [
...
def _valor(fila: pd.Series, col: str) -> str:
    v = fila.get(col, "")
...
def clasificar_automatico(fuente: str, fila: pd.Series) -> tuple[str, str]:
    ...
    NO_AFOLU o REVISAR."""
    f = normalizar(fuente)
    nombre = _valor(fila, "nombre_proyecto")

    # -- campos estructurados, por fuente ---------------------------------
    if f == "gold_standard":
        met = _valor(fila, "sector__methodology")
        if met:
    ...
    if f == "renare":
        tipo = _valor(fila, "sector__tipo")
        act = _valor(fila, "sector__actividades")
        if tipo:
            if _match(RENARE_TIPO_AFOLU, tipo):
                return "AFOLU", f"tipo: {tipo[:60]}"
            # PDBC / MDL / NAMA pueden contener foresteria: se mira
            # 'actividades' antes de descartar.
            if act and _match(NOMBRE_AFOLU, act):
                return ETIQUETA_PENDIENTE, f"tipo={tipo[:30]}; actividades sugiere AFOLU"
    ...
    if f == "verra":
        scope = _valor(fila, "sector__sectoralScope") or _valor(fila, "sector__scope")
        if scope:
            if _match(VERRA_AFOLU, scope):
                return "AFOLU", f"sectoralScope: {scope[:60]}"
            return "NO_AFOLU", f"sectoralScope: {scope[:60]}"
        for col in [c for c in fila.index if str(c).startswith("sector__")]:
            val = _valor(fila, col)
...
def anotar_sector(
    ...
    """Anade la columna 'sector' a los eventos consolidados.
    ...
    mapa = dict(zip(tabla["clave"], tabla["clasificacion_final"].str.strip().str.upper()))

    ev = eventos.copy()
    ev["_clave"] = [
        clave_proyecto(f, n)
        for f, n in zip(ev["fuente"], ev["nombre_proyecto"])
    ]
    ev["sector"] = ev["_clave"].map(mapa).fillna("SIN_CLASIFICAR")

    pendientes = ev[~ev["sector"].isin(ETIQUETAS_VALIDAS)]
    if len(pendientes) > 0:
        detalle = (
            pendientes[["fuente", "nombre_proyecto", "sector"]]
            .drop_duplicates()
            .to_string(index=False)
        )
        mensaje = (
            f"\n{len(pendientes)} evento(s) sin clasificacion sectorial valida:\n"
            f"{detalle}\n\n"
            f"Edita 'clasificacion_final' en {ruta} (valores permitidos: "
