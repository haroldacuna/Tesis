...
def _write_empty_outputs(reason: str) -> None:
    ...
    by_total = pd.DataFrame(
        columns=["COD_DANE", "NOMBRE_MPI", "DPTO_CNMBR", "loss_pixels", "loss_area_ha"]
    )
    ...
    print(f"Written empty yearly loss to: {OUT_BY_YEAR}")
    print(f"Written empty total loss to: {OUT_TOTAL}")
...
def main() -> None:
    ...
    if not LOSSYEAR_RASTER.exists():
        _write_empty_outputs("GFC lossyear raster not found")
        return
    ...
    if by_year.empty:
        _write_empty_outputs("no loss observations extracted (check raster coverage and CRS alignment)")
        return
